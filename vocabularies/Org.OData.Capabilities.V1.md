...
If T contains no non-containment navigation property, the capabilities annotation
applies to every entity that can be addressed by a resource path matching T,
even when it is addressed via another resource path.

Capabilities annotations where T contains at least one non-containment navigation property
are specific to resource paths matching T; their restrictions take precedence over restrictions
from capabilities annotations where T contains no non-containment navigation property.
