...
[PayloadExpressionSupport](Org.OData.Capabilities.V1.xml#L616)|[PayloadExpressionSupportType](#PayloadExpressionSupportType)|<a name="PayloadExpressionSupport"></a>Whether expressions for parameters in action payloads [OData-JSON, section 18] and `$query` payloads [OData-URL, section 4.17] are supported<br><p>Expressions can occur in payload properties that represent</p> <ul> <li>action parameters if the property name has the suffix <code>@expression</code> or</li> <li>function parameters if the property name has the suffix <code>@expression</code> in <code>\$query</code> payloads</li> <li>system query options (without suffix) in <code>\$query</code> payloads.</li> </ul> 
