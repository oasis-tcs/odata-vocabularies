...
[IsSignature](./Org.OData.Core.V1.xml#L561:~:text=<Term%20Name="-,IsSignature,-")|[Tag](#Tag)|<a name="IsSignature"></a>The tagged action or function cannot be invoked, it only defines a signature<br>Actions or functions with this tag MUST NOT have more than one overload. Services that want to implement the signature define one or more action or function overloads with an equivalent signature and annotate them with the term [`ImplementsSignature`](#ImplementsSignature). Equivalence of signatures means that clients which understand the CSDL definition of the tagged action or function must be able to make correct requests to any of its implementations based solely on the CSDL definition of that implementation. (See [this example](../examples/Org.OData.Core.V1.IsSignature-vocabsample.md).)
[ImplementsSignature](./Org.OData.Core.V1.xml#L572:~:text=<Term%20Name="-,ImplementsSignature,-")|String|<a name="ImplementsSignature"></a>The action or function overload implements the action or function [signature](#IsSignature) whose qualified name is the annotation value<br>This term can be applied to actions or functions as well as to properties or terms that identify an action or function.
[AnyStructure](./Org.OData.Core.V1.xml#L580:~:text=<Term%20Name="-,AnyStructure,-")|[Tag](#Tag)|<a name="AnyStructure"></a>Instances of a type are annotated with this tag if they have no common structure in a given response payload<br>The select-list of a context URL MUST be `(@Core.AnyStructure)` if it would otherwise be empty, but this instance annotation SHOULD be omitted from the response value.
