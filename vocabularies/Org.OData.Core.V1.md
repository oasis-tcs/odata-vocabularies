...
[IsSignature](./Org.OData.Core.V1.xml#L561:~:text=<Term%20Name="-,IsSignature,-")|[Tag](#Tag)|<a name="IsSignature"></a>The tagged action or function cannot be invoked, it only defines a signature<br>Actions or functions with this tag MUST NOT have more than one overload. Services that want to implement the signature define an action or function with an equivalent signature and annotate it with the term [`ImplementsSignature`](#ImplementsSignature). Equivalence of signatures means that clients which understand the CSDL definition of the tagged action or function must be able to make correct requests to any of its implementations based solely on the CSDL definition of that implementation. (See [this example](../examples/Org.OData.Core.V1.IsSignature-equivalence.md).)
