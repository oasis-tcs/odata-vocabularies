...
[AnyStructure](Org.OData.Core.V1.xml#L561)|[Tag](#Tag)|<a name="AnyStructure"></a>Instances of a type are annotated with this tag if they have no common structure in a given response payload<br>The select-list of a context URL MUST contain `@Core.AnyStructure` if it contains no structural properties, but this instance annotation SHOULD be omitted from the response value.
