...
[ExplicitOperationBindings](./Org.OData.Core.V1.xml#L516:~:text=<Term%20Name="-,ExplicitOperationBindings,-")|\[String\]|<a name="ExplicitOperationBindings"></a>The qualified names of explicitly bound operations that are supported on the target model element. These operations are in addition to any operations bound to the type of the target model element not annotated with RequiresExplicitBinding.
