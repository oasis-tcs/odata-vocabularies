...
module.exports.csdl2markdown = function (filename, csdl, referenced = {}) {
  ...
  function header(schema, filename) {
    ...
    if (!schema.$$namespace.endsWith(".sample")) {
  ...
  function operationSection(overloads) {
    ...
    overloads.forEach((o) => {
      ...
      if (getAnnotation(o, voc.Core.IsSignature))
        lines.push(
          "**Signature only" +
            (o[voc.Core.Description] || longDescription ? ":**" : "**"),
        );
      else if (getAnnotation(o, voc.Core.ImplementsSignature))
        lines.push(
          signatureLink(o, o[voc.Core.Description] || longDescription),
        );
  ...
  function descriptionInTable(modelElement) {
    ...
    return depr
      ...
      : (
          ...
          (long ? "<br>" + escape(long) : "") +
          (getAnnotation(modelElement, voc.Core.ImplementsSignature)
            ? "<br>" + signatureLink(modelElement)
            : "") +
          applicableTermsList(
  ...
  function sourceLink(modelElement, parent = false) {
    ...
    const italic =
      ...
      getAnnotation(modelElement, voc.Core.IsSignature) ||
      parent
        ? "*"
        : "";
    return (
  ...
  function link(ref) {
    ...
    return { showLink: customType, file: customFile, name: np.name };
  }
  ...
  function typeLink(modelElement) {
    let { showLink, file, name } = link(modelElement.$Type || "Edm.String");
  ...
  /**
   * Get annotation independent of the qualifier
   * @param {object} modelElement
   * @param {boolean} term @ns.term
  ...
  /**
   * Construct link to documentation of a model element's type in markdown
   * @param {object} modelElement model element with ImplementsSignature annotation
   * @param {boolean} withColon
   * @return {string} link
   */
  function signatureLink(modelElement, withColon) {
    const { showLink, file, name } = link(
      getAnnotation(modelElement, voc.Core.ImplementsSignature),
    );
    return (
      "**Implements " +
