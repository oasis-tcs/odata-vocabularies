...
module.exports.csdl2markdown = function (filename, csdl) {
    ...
    function typeSection(type) {
        ...
        if (longDescription) {
            lines.push('');
            lines.push(escape(longDescription));
        }
    ...
    function allowedValues(typeDefinition) {
        ...
        if (values) {
            ...
            values.forEach(v => {
                v.$$name = v.Value;
                lines.push(sourceLink(v) + '|' + descriptionInTable(v));
            });
            // typeDefinition.enum = values.map(record => record.Value);
        }
    ...
    function propertyLines(type, parent = false) {
        ...
        Object.keys(type).filter(name => isIdentifier(name)).forEach(name => {
            const p = type[name];
            p.$$name = name;
            lines.push(sourceLink(p, parent) + '|' + typeLink(p) + '|' + descriptionInTable(p));
        });
    ...
    function descriptionInTable(modelElement) {
        ...
        //TODO: experimental, example, deprecated
    ...
    function typeLink(modelElement) {
        ...
        return (modelElement.$Collection ? '\\[' : '')
            ...
            + (customType ? '](' + (externalType ? index.namespace[np.qualifier] + '.md' : '') + '#' + np.name + ')' : '')
