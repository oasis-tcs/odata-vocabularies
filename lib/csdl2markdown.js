...
module.exports.csdl2markdown = function (filename, csdl, referenced = {}) {
  ...
  function operationSection(overloads) {
    ...
    overloads.forEach((o) => {
      ...
      o.$Parameter.forEach(function(p, i) {
        ...
        if (depr)
          desc = depr.Description;
        else
          desc = descriptionInTable(p);
        lines.push(
           sourceLink(p) +
             ...
             typeLink(p) +
             "|" +
             (o.$IsBound && i === 0 ? "**Binding parameter" + (desc ? ":** " : "**") : "") + desc
        );
      });
      if (o.$ReturnType)
        lines.push(
  ...
  /**
   * Mark as experimental or deprecated or ...
   * @param {object} modelElement Model element
   * @return {string} Marker for experimental or deprecated or ...
   */
    ...
    return (
      (modelElement[voc.Common.Experimental]
        ? " *(" +
          ...
          ")*"
        : "") +
      (modelElement[voc.Core.AppliesViaContainer]
        ? " *(" +
          typeLink({ $Type: "Org.OData.Core.V1.AppliesViaContainer" }) +
          ")*"
        : "") +
  ...
  function descriptionInTable(modelElement) {
    ...
    return depr
      ...
      : escape(text) +
          ...
          allowedTermsList(
            modelElement[voc.Validation.AllowedTerms] || []
          );
  }
  ...
  function allowedTermsList(allowedTerms) {
    const text = [];
    if (allowedTerms.length > 0) text.push("<p>Allowed terms:<ul>");
    allowedTerms.forEach((term) => {
      text.push(`<li>${typeLink({ $Type: term })}</li>`);
    });
    if (allowedTerms.length > 0) text.push("</ul></p>");
    return text.join("");
  }
