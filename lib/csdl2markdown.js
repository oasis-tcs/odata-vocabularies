...
module.exports.csdl2markdown = function (filename, csdl, referenced = {}) {
  ...
  function header(schema, filename) {
    ...
    if (name.toLowerCase() !== name) lines.push("# " + name + " Vocabulary");
  ...
  function operationSection(overloads) {
    ...
    overloads.forEach((o) => {
      ...
      if (getAnnotation(o, voc.Core.IsInterface))
        lines.push(
          "**Interface" +
            (o[voc.Core.Description] || longDescription ? ":**" : "**"),
        );
      else if (getAnnotation(o, voc.Core.ImplementsInterface))
        lines.push(
          interfaceLink(o, o[voc.Core.Description] || longDescription),
        );
  ...
  function descriptionInTable(modelElement) {
    ...
    return depr
      ...
      : escape(text) +
          ...
          (long ? "<br>" + escape(long) : "") +
          (getAnnotation(modelElement, voc.Core.ImplementsInterface)
            ? "<br>" + interfaceLink(modelElement)
            : "") +
          applicableTermsList(
  ...
  /**
   * Construct link to documentation of a model element's type in markdown
   * @param {object} modelElement model element with ImplementsInterface annotation
   * @param {boolean} withColon
   * @return {string} link
   */
  function interfaceLink(modelElement, withColon) {
