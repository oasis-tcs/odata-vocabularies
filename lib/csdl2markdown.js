...
module.exports.csdl2markdown = function (filename, csdl, referenced = {}) {
  ...
  function signature(overload) {
    let s =
      overload.$Parameter
        .map((p) => paramDesc(p.$Name + ":&nbsp;" + typeLink(p), p))
        .join(", ") +
      (overload.$ReturnType
        ? paramDesc(" &rarr;&nbsp;" + typeLink(overload.$ReturnType), overload.$ReturnType)
        : "");
    return s;
  }

  /**
   * Append description to part of a signature
   * @param {string} s Signature before appending description
   * @param {object} p Parameter or return type from action or function overload
   * @return {string} Signature with description
   */
  function paramDesc(s, p) {
    var desc = p["@Core.Description"] || "";
    if (p["@Core.LongDescription"]) {
      if (desc) desc += " ";
      desc += p["@Core.LongDescription"];
    }
    if (desc) s += " (" + desc + ")";
    return s;
  }
