...
module.exports.csdl2markdown = function (filename, csdl, referenced = {}) {
  ...
  function signature(overload) {
    let s =
      overload.$Parameter
        .map((p) => p.$Name + ":&nbsp;" + typeLink(p))
        .join(", ") +
      (overload.$ReturnType
        ? " &rarr;&nbsp;" + typeLink(overload.$ReturnType)
        : "");
    return s;
  }
    ...
    return (
      ...
      (modelElement[voc.Core.AppliesViaContainer]
        ? " *(" +
          ...
          ")*"
        : "") +
      (modelElement[voc.Common.StructuralPropertyPath]
        ? " *(" +
          typeLink({ $Type: "com.sap.vocabularies.Common.v1.StructuralPropertyPath" }) +
          ")*"
        : "") +
  ...
  function vocabularies(alias) {
    const terms = {
      Common: ["Experimental","StructuralPropertyPath"],
      Core: [
        "Description",
        "Example",
        "IsMediaType",
